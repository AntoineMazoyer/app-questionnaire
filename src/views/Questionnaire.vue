      ...
      <v-content>
        <v-container fluid fill-height>
          <v-layout align-center justify-center>
            <v-flex xs12 sm8 md4>
              <h1 style="margin-bottom: 5%;">Test de {{firstname}} {{lastname}} (appartenant à l'entreprise : {{company}})</h1>
              ...
              <v-btn v-if="this.questions.length > this.questionIndex + 1" @click="am_next">Question suivante</v-btn>
              <v-btn v-if="questionIndex > 0" @click="am_prev">Question Précédente</v-btn>
              <v-btn v-if="this.questionIndex+1 == this.questions.length" @click="am_calcResponse">Terminer le test</v-btn>
            </v-flex>
          </v-layout>
          </v-container>
        </v-content>
...
export default {
  ...
  methods: {
    ...
    // On compare les réponses envoyés par l'utilisateur ainsi que les bonnes réponses prédéfinies dans le fichier JSON
    am_calcResponse(){
      const rightAnswer = this.questions.map(questionIndex => {
        return questionIndex.userAnswer.map(currentAnswer => {
          return questionIndex.goodAnswer.find(currgoodAnswer => {
            return (
              currgoodAnswer.idAnswer === currentAnswer.id &&
              currgoodAnswer.value === currentAnswer.value
            );
          });
        });
      });
      let totalGoodAnswer = 0;
      rightAnswer.forEach(questions => {
        let isGoodQuestion = true;
      ...
      console.log("Nombre total de bonnes réponses : ", totalGoodAnswer);
      router.push({
        name: "resultat",
        query: {
          totalGoodAnswer: this.totalGoodAnswer
        }
      });
    },
    // Index question
